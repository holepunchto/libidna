...
/**
 * Runs the converter against the test cases for verifying UTS #46
 * conformance, of which only those of nontransitional ToASCII apply.
 *
 * https://www.unicode.org/Public/idna/latest/IdnaTestV2.txt
 */

// The status codes of the flags that this sets to false, being
...
// Tells whether a column of status codes holds any code that the converter
...
main(int argc, char *argv[]) {
  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    ...
    if (unescape(source, &input) && unescape(to_ascii, &expected)) {
      int err = idna_url_to_ascii(utf8_string_view(&input), &actual);
      ...
      if (err == 0) {
        ...
        assert(idna_url_to_ascii(utf8_string_view(&actual), &again) == 0);
