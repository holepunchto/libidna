// Generates the character tables needed to convert a domain from the Unicode
// Character Database, as documented in <https://www.unicode.org/reports/tr44>.
//
// Usage:
//
...
// `idna__mapping_t`. Deviation is folded into valid as this only
...
fs.writeFileSync(
  header,
  `${banner}
#ifndef IDNA_TABLES_H
#define IDNA_TABLES_H

#include <stdint.h>
#include <utf.h>

#define IDNA_UNICODE_VERSION "${version}"
 ...
 */
typedef uint32_t idna__range_t;

/**
 * A block index narrows a search down to the entries that may hold a given code
 * point. The high \`21 - IDNA_BLOCK_SHIFT\` bits of the code point index
 * the block, and the entries of interest are those from the block's own index up
 * to and including that of the block after it.
 */
#define IDNA_BLOCK_SHIFT ${BLOCK_SHIFT}

#define IDNA_BLOCKS ${BLOCKS}

/**
 * The Bidi_Class values referenced by the Bidi rule of RFC 5893. Code points of
 * any other class are given \`idna__bidi_class_other\`, which no label of
 * a Bidi domain name may contain.
 *
 * https://www.rfc-editor.org/rfc/rfc5893#section-2
 */
typedef enum {
  idna__bidi_class_other = 0,
  idna__bidi_class_l = ${BIDI_CLASSES.L},
  idna__bidi_class_r = ${BIDI_CLASSES.R},
  idna__bidi_class_al = ${BIDI_CLASSES.AL},
  idna__bidi_class_an = ${BIDI_CLASSES.AN},
  idna__bidi_class_en = ${BIDI_CLASSES.EN},
  idna__bidi_class_es = ${BIDI_CLASSES.ES},
  idna__bidi_class_cs = ${BIDI_CLASSES.CS},
  idna__bidi_class_et = ${BIDI_CLASSES.ET},
  idna__bidi_class_on = ${BIDI_CLASSES.ON},
  idna__bidi_class_bn = ${BIDI_CLASSES.BN},
  idna__bidi_class_nsm = ${BIDI_CLASSES.NSM},
} idna__bidi_class_t;
 ...
 */
typedef enum {
  idna__joining_type_other = 0,
  idna__joining_type_left = ${JOINING_TYPES.L},
  idna__joining_type_right = ${JOINING_TYPES.R},
  idna__joining_type_dual = ${JOINING_TYPES.D},
  idna__joining_type_transparent = ${JOINING_TYPES.T},
} idna__joining_type_t;
 ...
 */
extern const idna__range_t idna__property_ranges[${propertyRanges.length}];

extern const uint16_t idna__property_blocks[IDNA_BLOCKS];

/**
 * The status values of the IDNA mapping table. Deviation is folded into valid
 * as this only ever performs nontransitional processing, which
 ...
 */
typedef enum {
  idna__status_valid = ${IDNA_VALID},
  idna__status_ignored = ${IDNA_IGNORED},
  idna__status_mapped = ${IDNA_MAPPED},
  idna__status_disallowed = ${IDNA_DISALLOWED},
} idna__status_t;
 ...
 * \`idna__mapping_data\`, packed together with the status as
...
extern const idna__mapping_t idna__mappings[${idnaRanges.length}];

extern const uint16_t idna__mapping_blocks[IDNA_BLOCKS];

/**
 * The mapping of every ASCII code point, indexed by the code point itself.
 */
extern const uint32_t idna__ascii_mappings[0x80];

extern const utf32_t idna__mapping_data[${idnaMappingData.data.length}];
...
const idna__range_t idna__property_ranges[${propertyRanges.length}] = {
${table(propertyRanges.map(packRange), 8)}};

const uint16_t idna__property_blocks[IDNA_BLOCKS] = {
${table(propertyBlocks.map(String), 12)}};

const idna__mapping_t idna__mappings[${idnaRanges.length}] = {
${table(
  idnaRanges.map(([c, v]) => `{${hex(c)}, ${hex(v)}}`),
  4
)}};

const uint16_t idna__mapping_blocks[IDNA_BLOCKS] = {
${table(idnaBlocks.map(String), 12)}};

const uint32_t idna__ascii_mappings[0x80] = {
${table(idnaAscii.map(hex), 8)}};

const utf32_t idna__mapping_data[${idnaMappingData.data.length}] = {
